...
| `id_ed25519_bdramassist` (on **BdRSrvAMI**, `bdr@192.168.100.20`, not this host) | `github.com:bDotRad/BdRAMAssist.git` | Read-only deploy key for BdRSrvAMI to pull BdRAMAssist, generated + installed 2026-08-26 (`gh repo deploy-key add` worked fine from this host this time, unlike the write-access key above). Named `id_ed25519_bdramassist` to match the pre-existing `id_ed25519_planbdrad` naming style already used on that machine, not the `<source>_to_<target>` convention below (BdRSrvAMI deviates from this doc's convention). Wired via a `github.com-bdramassist` Host alias in BdRSrvAMI's `~/.ssh/config` (that machine's bare `Host github.com` is already claimed by the PlanBdRad key). Repo cloned to `~/BdRAMAssist` on BdRSrvAMI (sibling of `~/PlanBdRad`, not under a `~/projects/` folder — BdRSrvAMI doesn't use that convention). |
...
| `bdrdev_to_bdrpiamiserver` | `bdr@10.10.10.20` (`BdRPiAMI`) | SSH into the new AMI Raspberry Pi (Ubuntu 24.04 aarch64, replaces the retired VM `BdRSrvAMI`). Generated 2026-08-27. Public half goes in the Pi's `~/.ssh/authorized_keys` via step 3 of `~/projects/BdRPiAMI-PROVISION.md`. `~/.ssh/config` alias `BdRPiAMI`. Genuine Pi, so the runbook/name may say "Pi". |
